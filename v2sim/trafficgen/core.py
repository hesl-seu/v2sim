...
class TrafficGenerator:
    def __init__(
        ...
        _, self.__end_time, __rnet_file, _ = GetTimeAndNetwork(self.__cfg.cfg)
        if __rnet_file is not None and Path(__rnet_file).exists():
            self.__cfg.net = str(Path(self.__cfg.cfg).parent / __rnet_file)
