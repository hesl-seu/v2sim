...
class ConvertBox(Tk):
    ...
    def select_input_folder(self):
        ...
        if folder:
            ...
            if self.output_folder.get() == "":
                p = Path(folder)
                if Path(folder.lower()).name.startswith("sumo_"):
                    p = p.parent / ("ux_" + p.name[5:])
                    p0 = Path(str(p))
                    i = 1
                    while p.exists():
                        p = p0.parent / ("ux_" + p0.name[3:] + f"({i})")
                        i += 1
                    self.output_folder.set(str(p))
                else:
                    self.output_folder.set(str(Path(folder) / (Path(folder).name + "_converted")))
    ...
    def first_program_completed(self, result):
        ...
        if answer:
            ...
            subprocess.run([sys.executable, str(exec_dir), "-d", self.output_path])
        else:
            self.reset_ui()
