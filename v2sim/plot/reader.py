...
class StatisticsNotSupportedError(Exception):
    pass
...
class ReadOnlyStatistics(StaReader):
    def has_FCS(self) -> bool:
        return self.__fcs_head is not None

    def has_SCS(self) -> bool:
        return self.__scs_head is not None

    def has_GS(self) -> bool:
        return self.__gs_head is not None

    def has_EV(self) -> bool:
        return FILE_EV in self

    def has_GEN(self) -> bool:
        return FILE_GEN in self

    def has_BUS(self) -> bool:
        return FILE_BUS in self

    def has_LINE(self) -> bool:
        return FILE_LINE in self

    def has_PVW(self) -> bool:
        return FILE_PVW in self

    def has_ESS(self) -> bool:
        return FILE_ESS in self
    ...
    @property
    def GS_head(self) -> List[str]:
        assert self.__gs_head is not None, "GS propertirs not supported"
        if not isinstance(self.__gs_head, list):
            self.__gs_head = self.__loadhead(FILE_GS)
            self.__gs_head.sort(key=_parse_val)
        return self.__gs_head
    ...
    def GS_attrib_of(self, gs: str, attrib: str) -> SegFunc:
        assert attrib in GS_ATTRIB, f"Invalid gas station property: {attrib}"
        if gs == "<sum>":
            d = [self.GetColumn(FILE_GS, c + "#" + attrib) for c in self.GS_head]
            return SegFunc.qs(d)
        return self.GetColumn(FILE_GS, gs + "#" + attrib)
    ...
    def GS_count_of(self, gs: str) -> SegFunc:
        """Gas station vehicle count"""
        return self.GS_attrib_of(gs, "cnt")
    ...
    def EV_net_cost_of(self, veh: str) -> SegFunc:
        """EV net cost"""
        return self.GetColumn(FILE_EV, veh + "#cost") - self.GetColumn(
            FILE_EV, veh + "#earn"
        )
...
__all__ = ["ReadOnlyStatistics", "StatisticsNotSupportedError"]