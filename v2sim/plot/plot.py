...
from typing import Any, Dict, List, Literal, Optional, Union, Iterable, Tuple
import matplotlib
matplotlib.use("agg")
...
class AdvancedPlot:
    ...
    def get_series(self, series:str) -> SegFunc:
        ...
        if domain == "gen_total_active":
            ...
            elif domain == "gs_count":
                d = se.GS_count_of(val)
            elif domain == "ev_soc":
    ...
    def quick_gs(self,tl:int,tr:int,gs_name:str,cnt:bool,save_to:str="",res_path:str="results"):
        self.new_fig(tl,tr)
        assert cnt == True
        self.add_data("{"+f"{res_path}|gs_count|{gs_name}"+"}","Veh. Count",color="gray")
        self.yleft_label(Lang.PLOT_YLABEL_COUNT)
        if self.plot_title: self.title(Lang.PLOT_GS.format(gs_name))
        self.legend()
        self.x_label(Lang.PLOT_XLABEL_TIME)
        p = Path(res_path) / "figures"
        p.mkdir(parents=True,exist_ok=True)
        if gs_name == "<sum>": gs_name = "sum"
        if save_to == "": save_to = str(p / f"gs_{gs_name}.{self.pic_ext}")
        self.save(save_to)
