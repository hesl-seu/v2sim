...
class Analyzer:
    ...
    def od_analysis(s):
        ...
        for o,d in s.od_tt.keys():
            s.od_tt_ave[o,d] = np.average(s.od_tt[o,d]).item()
            s.od_tt_std[o,d] = np.std(s.od_tt[o,d]).item()
            s.od_tt_free[o,d] = dist_time[o.id, d.id]
            s.od_dist_total[o,d] = np.sum(s.od_dist[o,d])
            s.od_dist_min[o,d] = dist_space[o.id, d.id]
            s.od_dist_ave[o,d] = np.average(s.od_dist[o,d]).item()
            s.od_dist_std[o,d] = np.std(s.od_dist[o,d]).item()
    ...
    def link_analysis_coarse(s):
        ...
        for l in s.W.LINKS:
            ...
            if s.linkc_volume[l]:
                s.linkc_tt_ave[l] = np.average([t for t in l.traveltime_actual if t>0]).item()
                s.linkc_tt_std[l] = np.std([t for t in l.traveltime_actual if t>0]).item()
